...
| RAGAS / 50-question eval | Closed | `backend/tests/golden_qa.json`, `run_ragas.py` |
| OpenAI GPT-4o-mini provider | Closed | `LLM_PROVIDER=openai` in `agents/llm.py` |
| Analytical demo chip | Closed | 4th suggestion in `SuggestionChips.tsx` |
...
- [ ] Run `python scripts/reingest_all.py` on Railway after deploy
- [ ] `/health` shows increased `collection_count` including tickets
- [ ] Query `"Summarize all P1 deployment tickets this month"` returns `doc_type: ticket` citations
- [ ] Run `python tests/run_ragas.py --api-url <backend-url>` and update README metrics table
- [ ] Live demo: factual → analytical → edge query

## Q&A line for judges

> We implemented equivalent agent orchestration in FastAPI for a leaner Railway deploy. Runtime trace and RAGAS eval cover prototype observability; LangSmith is on the production roadmap.
